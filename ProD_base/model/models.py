...
class HFBertEncoder(ErnieModel):
    def __init__(self, config):
        ErnieModel.__init__(self, config)
    ...
    def init_encoder(cls, args, dropout: float = 0.1, model_type=None):
        ...
        cfg = ErnieConfig.from_pretrained(model_type)
        if dropout != 0:
        ...
        # if cfg.num_hidden_layers > 12:
        #     L_cfg = BertConfig.from_pretrained("bert-large-uncased")
        #     L_cfg.num_hidden_layers = cfg.num_hidden_layers
        #     if dropout != 0:
        #         L_cfg.attention_probs_dropout_prob = dropout
        #         L_cfg.hidden_dropout_prob = dropout
        #     if int(transformers.__version__.split('.')[0]) >= 3:
        #         L_cfg.gradient_checkpointing = args.gradient_checkpointing
        #     return cls.from_pretrained("bert-large-uncased", config=L_cfg)
...
class HFDistillBertEncoder(DistilBertModel):
    def __init__(self, config):
    ...
    def init_encoder(cls, args, dropout: float = 0.1, model_type=None):
        ...
        if model_type is None:
            model_type = args.model_type
        cfg = DistilBertConfig.from_pretrained(model_type)
        if dropout != 0:
            cfg.attention_probs_dropout_prob = dropout
            cfg.hidden_dropout_prob = dropout
        if int(transformers.__version__.split('.')[0]) >= 3:
...
class BiBertEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.model_type == "distilbert-base-uncased":
            self.question_model = HFDistillBertEncoder.init_encoder(args)
            if hasattr(args, 'share_weight') and args.share_weight:
                self.ctx_model = self.question_model
            else:
                self.ctx_model = HFDistillBertEncoder.init_encoder(args)
        else:
...
class ColBERT(nn.Module):
    def __init__(self, args, similarity_metric='cosine'):
    ...
    def score(self, query_ids, attention_mask_q, input_ids_a, attention_mask_a):
        ...
        ctx_max_len = D_hidden.shape[1]
        dim = D_hidden.shape[2]
        Q_num = Q_hidden.shape[0]
        ctx_hidden = D_hidden.view(-1, dim)
        if args.similarity_metric == 'cosine':
            return ((Q_hidden @ ctx_hidden.permute(1, 0)).view(
...
class BiEncoderNllLoss(object):
    def calc(
        ...
        """
        # print("q_vectors:", q_vectors)
        # print("ctx_vectors:", ctx_vectors)
        # q_vectors = torch.nn.functional.normalize(q_vectors, p=2, dim=1)
        ...
        loss = F.nll_loss(
            softmax_scores,
            torch.tensor(positive_idx_per_question).to(softmax_scores.device).detach(),
            reduction="mean",
        )
        ...
        # print("correct_predictions_count:", correct_predictions_count)
        # print(softmax_scores.shape)
        # print(positive_idx_per_question)

        if loss_scale:
            loss.mul_(loss_scale)
        # print("correct:", correct_predictions_count*1.0/q_vectors.size(0))
...
class ColBERTNllLoss(object):
    def calc(
        ...
        Q_num = q_hidden.shape[0]
        q_max_len = q_hidden.shape[1]
        scores = torch.einsum('qin,pjn->qipj', q_hidden, ctx_hidden)
        mask_index = (~ctx_mask.bool()).unsqueeze(0).unsqueeze(0).repeat(Q_num, q_max_len, 1, 1)
        scores[mask_index] = -9999
